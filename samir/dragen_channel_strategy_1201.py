...
class DragenStrategyExact(CtaTemplate):
    ...
    def on_init(self) -> None:
        ...
        self.load_bar(100, Interval.DAILY, None, True)
...
# -------------------- 逐个计算 --------------------
    ...
    #         try:
    #             bar_attrs = vars(bar)
    #             print("    bar attrs (vars):")
    #             for k, v in bar_attrs.items():
    #                 print(f"      {k}: {v}")
    #         except Exception:
    #             print("    bar dir():", dir(bar))
    #     except Exception as e:
    #         print("DEBUG 打印 bar 属性时发生异常:", e)

    #     # --- 继续原有逻辑：把 bar 更新到 ArrayManager 等 ---
    
    #     self.am.update_bar(bar)
    #     if not self.am.inited:
    #         return

    #     if len(self.am.close) < self.belt_smooth_period:
    #         return
    ...
    #     # 打印数据信息用于调试
    #     print(f"=== 数据信息 ===")
    #     print(f"数据总长度: {len(highs)}")
    #     print(f"最新Bar时间: {bar.datetime}")
    #     print(f"最新5个High: {highs[-5:]}")
    #     print(f"最新5个Low: {lows[-5:]}")
    #     print(f"最新5个Close: {closes[-5:]}")

    #     belt2, belt3, belt4, slld_0, slld_8 = self._compute_belt_exact(highs, lows)
    #     xma3_1, xma2_1 = self._compute_xma_exact(highs, lows, closes)
